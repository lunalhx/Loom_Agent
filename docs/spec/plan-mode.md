...
- The protocol semantics of Plan Submission are decided, but its concrete model-output wire encoding has not been selected. This spec does not invent a tag or payload shape; that implementation-blocking contract must be decided separately before coding begins.
